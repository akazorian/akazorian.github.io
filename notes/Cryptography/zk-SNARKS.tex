...
$$\bigl\lvert \Pr[D(X) = 1] - \Pr[D(U) = 1]\bigr\rvert = \text{ negl}(k)$$
...
\textbf{Definition:} \textit{A proof system is a pair of algorithms $(P, V)$ for a problem $\mathcal{P}$ such that below properties are satisfied:}
    \begin{enumerate}
        ...
        $$\Pr\bigl[V(x, \pi) = 1 \big\vert \pi = P(x)\bigr] \geq \frac{2}{3}$$
        ...
        $$\Pr\bigl[V(x, \pi) = 1 \big\vert \pi = P^*(x)\bigr] \leq \frac{1}{3}$$
    \end{enumerate}
...
$$\Pr\bigr[\begin{array}{c} (x, \pi) \leftarrow P^*(z) \\ V(x, \pi) = 1\end{array} \wedge \begin{array}{c} (x, w) \leftarrow E^{P^*}(z) \\ w \text{ is not a solution for } x \text{ under problem} \mathcal{P}\end{array}\bigl]$$
\\
