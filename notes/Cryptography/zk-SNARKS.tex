...
$$\biggl\lvert \Pr[D(X) = 1] - \Pr[D(U) = 1]\biggr\rvert = \text{ negl}(k)$$
...
\textbf{Definition:} \textit{A proof system is a pair of algorithms $(P, V)$ for a problem $\mathcal{P}$ such that below properties are satisfied:}
    \begin{enumerate}
        ...
        $$\Pr\biggl[V(x, \pi) = 1 \bigg\vert \pi = P(x)\biggr] \geq \frac{2}{3}$$
        ...
        $$\Pr\biggl[V(x, \pi) = 1 \bigg\vert \pi = P^*(x)\biggr] \leq \frac{1}{3}$$
    \end{enumerate}
...
$$\Pr\biggr[\begin{array}{c} (x, \pi) \leftarrow P^*(z) \\ V(x, \pi) = 1\end{array} \wedge \begin{array}{c} (x, w) \leftarrow E^{P^*}(z) \\ w \text{ is not a solution for } x \text{ under problem } \mathcal{P}\end{array}\biggl]$$
\\
